...
export default function Pricing() {
    ...
    return (
        <section
            id="pricing"
            className="py-6 bg-white dark:bg-black transition-colors duration-300"
        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {plans.map((plan, index) => (
                        <motion.div
                            ...
                            } hover:border-black/50 dark:hover:border-white/50 transition-all duration-300 ${
                        ...
                        >
                            ...
                            <h3
                                className={`text-xl font-medium mb-2 ${plan.highlight ? 'text-white dark:text-black' : 'text-black dark:text-white'}`}
                            >
                                {plan.name}
                            </h3>
                            <div className="flex items-baseline gap-1 mb-2">
                                <span
                                    className={`text-4xl font-light ${plan.highlight ? 'text-white dark:text-black' : 'text-black dark:text-white'}`}
                                >
                                    {plan.price}
                                </span>
                                {plan.period && (
                                    <span
                                        className={`text-sm font-normal ${plan.highlight ? 'text-gray-400 dark:text-gray-600' : 'text-gray-500 dark:text-gray-400'}`}
                                    >
                                        {plan.period}
                                    </span>
                                )}
                            ...
                            <p
                                className={`text-sm mb-8 ${plan.highlight ? 'text-gray-400 dark:text-gray-600' : 'text-gray-500 dark:text-gray-400'}`}
                            >
                                {plan.description}
                            </p>
                            <ul className="space-y-4 mb-8 flex-1">
                                {plan.features.map((feature) => (
                                    ...
                                    >
                                        <Check
                                            className={`h-4 w-4 mr-3 flex-shrink-0 ${plan.highlight ? 'text-white dark:text-black' : 'text-black dark:text-white'}`}
                                        />
                                        {feature}
                                    </li>
                                ))}
