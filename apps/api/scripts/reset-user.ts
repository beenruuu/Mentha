...
async function resetUser() {
    ...
    console.log(
        `   Current credits: ${targetUser.credit_balance}, daily_quota: ${targetUser.daily_quota}`,
    );
    ...
    const userProjects = await db
        ...
        .from(projects)
        .where(eq(projects.user_id, targetUser.id));
    ...
    if (projectIds.length > 0) {
        ...
        const projectKeywords = await db
            ...
            .from(keywords)
            .where(inArray(keywords.project_id, projectIds));
        ...
        if (keywordIds.length > 0) {
            ...
            const jobs = await db
                ...
                .from(scanJobs)
                .where(inArray(scanJobs.keyword_id, keywordIds));
            ...
            // 7. Delete scan jobs
            const { rowCount: jobDel } = await db
                .delete(scanJobs)
                .where(inArray(scanJobs.keyword_id, keywordIds));
        ...
        const { rowCount: runDel } = await db
            .delete(scanRuns)
            .where(inArray(scanRuns.project_id, projectIds));
        console.log(`   🗑️  Deleted ${runDel} scan runs`);

        // 9. Delete keywords
        const { rowCount: kwDel } = await db
            .delete(keywords)
            .where(inArray(keywords.project_id, projectIds));
        console.log(`   🗑️  Deleted ${kwDel} keywords`);

        // 10. Delete projects
        const { rowCount: projDel } = await db
            .delete(projects)
            .where(eq(projects.user_id, targetUser.id));
    ...
    const { rowCount: txDel } = await db
        .delete(creditTransactions)
        .where(eq(creditTransactions.user_id, targetUser.id));
    ...
    console.log(
        `   Credits: ${verifiedUser.credit_balance} | Daily Quota: ${verifiedUser.daily_quota}`,
    );
