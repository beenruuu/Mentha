...
export const domains = pgTable(
    ...
    (table) => ({
        ...
        verificationMethodCheck: check(
            'verification_method_check',
            sql`verification_method IN ('dns_txt', 'meta_tag', 'file')`,
        ),
        sslStatusCheck: check(
            'ssl_status_check',
            sql`ssl_status IN ('pending', 'active', 'error')`,
        ),
        proxyModeCheck: check(
            'proxy_mode_check',
            sql`proxy_mode IN ('off', 'dns_only', 'proxied')`,
        ),
        platformTypeCheck: check(
            'platform_type_check',
...
// =============================================================================
export const aiFirewallRules = pgTable(
    'ai_firewall_rules',
    {
        id: uuid('id').defaultRandom().primaryKey(),
        tenant_id: uuid('tenant_id').notNull(),
    ...
    (table) => ({
        ...
        actionCheck: check(
            'action_check',
            sql`action IN ('allow', 'block', 'rate_limit', 'serve_llms_txt')`,
        ),
        purposeCheck: check(
            'bot_purpose_check',
            sql`bot_purpose IN ('training', 'search', 'preview', 'unknown')`,
        ),
    }),
);
...
// =============================================================================
export const edgeInjectionConfig = pgTable(
