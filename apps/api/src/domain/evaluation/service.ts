...
export interface EvaluationRequest {
    ...
    competitors: string[];
    /**
     * Context to disambiguate competitors (e.g. "Action" -> "Dutch retailer, not verb")
     * Key: Competitor Name, Value: Context/Constraints
     */
 ...
 * Uses GPT-4o-mini for cost-effective, reliable evaluation
 */
export class EvaluationService {
    ...
    /**
     * Evaluate an LLM response for brand metrics
    ...
    async evaluate(request: EvaluationRequest): Promise<EvaluationResult> {
        ...
        try {
            ...
            if (!result.success) {
                ...
                // Attempt auto-correction (one retry)
...
- recommendation_type:
  ...
  - "absent": Brand is not mentioned at all`;
    ...
    /**
     * Build the user prompt with the specific evaluation request
     */
...
Provide your evaluation as JSON. Remember the Entity Resolution rules: do NOT count false positives if the context doesn't match.`;
    ...
    /**
     * Attempt to auto-correct a malformed evaluation response
     */
...
Return ONLY the corrected JSON, nothing else.`;
        ...
        const result = EvaluationResultSchema.parse(parsed); // Will throw if still invalid
