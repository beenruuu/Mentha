...
export class GeminiProvider implements ISearchProvider {
    ...
    async search(query: string, options?: SearchOptions): Promise<SearchResult> {
        ...
        logger.debug('Gemini API request', { model: this.defaultModel, queryLength: query.length });
    ...
    async testConnection(): Promise<boolean> {
        ...
        try {
            ...
            logger.error('Gemini connection test failed', { error: (error as Error).message });
            return false;
        }
    }
}
